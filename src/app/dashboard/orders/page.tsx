...
import { Package, ChevronRight, Clock, Truck, CheckCircle, XCircle, CreditCard, MapPin, ChevronDown, ChevronUp, ExternalLink, Loader2 } from "lucide-react";
...
export default function OrdersPage() {
  ...
  const loadOrders = async () => {
    ...
    const email = user?.email || localStorage.getItem('beliseken_user_email');
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-brand-muted text-sm mt-1">{loading ? "Memuat..." : `${orders.length} pesanan`}</p>
            </div>
            <Link href="/products" className="px-4 py-2 bg-brand text-white text-sm font-semibold rounded-lg hover:bg-brand-dark transition-colors">
              + Belanja Lagi
            </Link>
          </div>
                      ...
                      <div className="border-t border-brand-border p-4 sm:p-5 bg-brand-gray/20">
                          ...
                          <div className="space-y-3">
                            {order.items.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-3">
                                ...
                                <p className="text-sm font-bold text-brand">{formatPrice(item.subtotal)}</p>
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                          <div className="bg-white rounded-xl p-4 border border-brand-border">
                            ...
                            <p className="text-xs text-brand-muted">{order.address?.city}, {order.address?.province} {order.address?.postcode}</p>
                          </div>
                          <div className="bg-white rounded-xl p-4 border border-brand-border">
                        ...
                        <div className="bg-white rounded-xl p-4 border border-brand-border mb-4">
                          ...
                          <div className="space-y-2">
                            {[...order.statusHistory].reverse().map((h, i) => {
                              const hc = statusConfig[h.status as OrderStatus] || statusConfig.pending;
                              const HIcon = hc.icon;
                              return (
                                <div key={i} className="flex items-start gap-3">
                                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${i === 0 ? hc.color : "bg-gray-100 text-gray-400"}`}>
                                    <HIcon size={12} />
                                  </div>
